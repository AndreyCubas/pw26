from django.urls import path
from django.views.generic import RedirectView
from .views import (
    CadastroView,
    ContatoView,
    DashboardView,
    GastoCreateView,
    GastoDeleteView,
    GastoDetailView,
    GastoUpdateView,
    GastosView,
    MetaCreateView,
    MetaAdicionarValorView,
    MetaDeleteView,
    MetaUpdateView,
    MetasView,
    RecursosView,
    RelatoriosView,
    SaldoCreateView,
    SaldoDeleteView,
    SaldoUpdateView,
    SaldoView,
    SobreView,
    UsuarioLoginView,
    UsuarioLogoutView,
    InicioView,
)

urlpatterns = [
    #path("", InicioView.as_view(), name="inicio"),
    path("dashboard/", DashboardView.as_view(), name="dashboard"),
    path("gastos/", GastosView.as_view(), name="gastos"),
    path("gastos/novo/", GastoCreateView.as_view(), name="gasto_novo"),
    path("gastos/<int:pk>/editar/", GastoUpdateView.as_view(), name="gasto_editar"),
    path("gastos/<int:pk>/excluir/", GastoDeleteView.as_view(), name="gasto_excluir"),
    path("gastos/<int:pk>/", GastoDetailView.as_view(), name="gasto_detalhe"),
    path("metas/", MetasView.as_view(), name="metas"),
    path("metas/nova/", MetaCreateView.as_view(), name="meta_nova"),
    path("metas/<int:pk>/adicionar-valor/", MetaAdicionarValorView.as_view(), name="meta_adicionar_valor"),
    path("metas/<int:pk>/editar/", MetaUpdateView.as_view(), name="meta_editar"),
    path("metas/<int:pk>/excluir/", MetaDeleteView.as_view(), name="meta_excluir"),
    path("saldo/", SaldoView.as_view(), name="saldo"),
    path("saldo/novo/", SaldoCreateView.as_view(), name="saldo_novo"),
    path("saldo/<int:pk>/editar/", SaldoUpdateView.as_view(), name="saldo_editar"),
    path("saldo/<int:pk>/excluir/", SaldoDeleteView.as_view(), name="saldo_excluir"),
    path("relatorios/", RelatoriosView.as_view(), name="relatorios"),
    path("recursos/", RecursosView.as_view(), name="recursos"),
    path("sobre/", SobreView.as_view(), name="sobre"),
    path("contato/", ContatoView.as_view(), name="contato"),
    path("cadastro/", CadastroView.as_view(), name="cadastro"),
    path("logout/", UsuarioLogoutView.as_view(), name="logout"),
    path("", RedirectView.as_view(url="login/"), name="home_redirect"),
    path("login/", UsuarioLoginView.as_view(), name="login"),
    
    
]
